...
如果依赖的代码没有提供tag, 可以通过指定commitId或分支名的方式进行依赖, 例如
...
defer语句用于保证指定的语句在函数结束时必定执行. 但这句话本身要先执行才可以, 所以通常要紧跟对应的语句, 否则容易产生BUG.
...
// 不可以在这里defer, 否则异常情况下根本不执行

// 正常情况下的业务逻辑
```

此外, 还需要注意, defer执行的时候, 会对函数的参数直接进行求值, 因此在后面对变量的值进行修改是不会影响defer函数的执行的, 例如
