...
通过该公式可以容易的知道, 对某个部分进行提升后, 从总体上看的提升效果是小于单独一个部分的. 因此如果想使整个系统加速, 则需要再相当大的部分进行加速.
...
超线程技术. 当CPU对某些硬件进行备份后(例如程序计数器和寄存器文件), 在浮点数运算单元依然仅保留一份的情况下, 可以在单个时钟周期内切换线程, 此时就使得一个核心相当于可以同时执行两个线程.

> 常规的处理器线程切换大约需要20000时钟周期


### 指令级并行

在1978年的 Intel 8086 处理器上, 执行一条指令需要3~10个时钟周期. 而现在的处理器可以在每个时钟周期执行2 ~ 4 条指令.

处理器通过流水线技术实现并行, 虽然一条指令可能需要几十个时钟周期, 但通过设置多级流水线, 依然可使得每个时钟周期都可以完成一条指令.

--------------

超标量(super-scalar)处理器. 可在一个时钟周期内执行超过1条指令的处理器.
