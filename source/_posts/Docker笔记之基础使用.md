...
联合文件系统（UnionFS）或层（Layer）技术允许将多个文件系统层叠在一起, 形成一个统一的视图. 每个镜像和容器都由一组层组成这些层可以共享和重用.


----

由于Docker高度依赖Linux内核的能力, 因此在非Linux平台, 无法直接运行Docker. 在Windows平台, 通常的解决方案是运行Windows的Linux子系统, 在子系统中启动Docker. 在Mac平台, 解决方案是先安装虚拟机, 在虚拟机中运行Linux系统, 最后在Linux系统中启动Docker(双层套娃结构). 由于Docker本质上是一种隔离, 因此无法直接在Docker中直接运行Windows或者Mac镜像.
