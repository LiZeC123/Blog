...
电势是静电场本身的一种属性。某点的电势等于将单位正电荷从该点移动到参考点（通常为无穷远或大地）时，**静电力**所做的功。

电动势是描述电源内部“非静电力”做功本领的物理量。电动势的大小等于将单位正电荷从电源的负极（低电势端）通过电源内部移动到正极（高电势端）时，**非静电力**所做的功。

> 正如两者名字上的差异一样, 两者的区别就在于是否为"动". 但其物理含义依然一致, 是单位电荷做工的度量
...
$$
\frac{\mathrm{d} W}{\mathrm{d} t}  = - \frac{\mathrm{d} }{\mathrm{d} t} \int_V \frac{1}{2} - \frac{1}{2} \frac{\partial }{\partial t} (\epsilon_0 E^2 + \frac{1}{\mu_0} B^2) \mathrm{d} \tau - \frac{1}{\mu_0} \oint_S (E \times B) \cdot \mathrm{d} a
$$
