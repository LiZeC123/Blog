...
| 函数 $f(x)$                         | 微分 $f'(x)$                          | 不定积分 $\displaystyle\int f(x)\mathrm dx$                                    |
| ----------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------ |
| $C$（常数）                         | $0$                                   | $Cx+C_0$                                                                       |
| $x^\mu$                             | $\mu x^{\mu-1}$                       | $\displaystyle\frac{x^{\mu+1}}{\mu+1}+C\ (\mu\neq-1)$                          |
| $\displaystyle\frac1x$              | $\displaystyle-\frac1{x^2}$           | $\ln                                                                           | x | +C$ |
| $e^x$                               | $e^x$                                 | $e^x+C$                                                                        |
| $a^x$                               | $a^x\ln a$                            | $\displaystyle\frac{a^x}{\ln a}+C$                                             |
| $\ln x$                             | $\displaystyle\frac1x$                | $x\ln x - x + C$                                                               |
| $\sin x$                            | $\cos x$                              | $-\cos x + C$                                                                  |
| $\cos x$                            | $-\sin x$                             | $\sin x + C$                                                                   |
| $\tan x$                            | $\sec^2x$                             | $-\ln                                                  \| \cos x        \| +C$ |
| $\cot x$                            | $-\csc^2x$                            | $\ln                                                   \| \sin x        \| +C$ |
...
| $\arcsin x$                         | $\displaystyle\frac1{\sqrt{1-x^2}}$   | $x\arcsin x+\sqrt{1-x^2}+C$                                                    |
| $\arccos x$                         | $\displaystyle-\frac1{\sqrt{1-x^2}}$  | $x\arccos x-\sqrt{1-x^2}+C$                                                    |
| $\arctan x$                         | $\displaystyle\frac1{1+x^2}$          | $x\arctan x-\frac12\ln(1+x^2)+C$                                               |
| $\sinh x$                           | $\cosh x$                             | $\cosh x + C$                                                                  |
| $\cosh x$                           | $\sinh x$                             | $\sinh x + C$                                                                  |
| $\displaystyle\frac1{1+x^2}$        | $\displaystyle-\frac{2x}{(1+x^2)^2}$  | $\arctan x + C$                                                                |
| $\displaystyle\frac1{\sqrt{1-x^2}}$ | $\displaystyle\frac x{(1-x^2)^{3/2}}$ | $\arcsin x + C$                                                                |
...
| 加减             | $(u\pm v)'=u'\pm v'$                                                               |
| 乘法（莱布尼茨） | $(uv)'=u'v+uv'$                                                                    |
| 除法             | $\displaystyle\left(\frac uv\right)'=\frac{u'v-uv'}{v^2}\ (v\neq0)$                |
| 复合函数（链式） | $\displaystyle\frac{\mathrm d}{\mathrm dx}f(u)=f'(u)\cdot u'$                      |
| 反函数           | $\displaystyle\frac{\mathrm dx}{\mathrm dy}=\frac1{\frac{\mathrm dy}{\mathrm dx}}$ |

---

| 法则                | 公式                                                                             |
| ------------------- | -------------------------------------------------------------------------------- |
| 常数提出            | $\displaystyle\int kf(x)\mathrm dx=k\int f(x)\mathrm dx$                         |
| 线性可加            | $\displaystyle\int [f(x)\pm g(x)]\mathrm dx=\int f\mathrm dx\pm\int g\mathrm dx$ |
| 微分与积分互逆      | $\displaystyle\left(\int f(x)\mathrm dx\right)'=f(x)$                            |
|                     | $\displaystyle\int f'(x)\mathrm dx=f(x)+C$                                       |
| 定积分牛顿-莱布尼茨 | $\displaystyle\int_a^b f(x)\mathrm dx=F(b)-F(a)$                                 |
...
=\int f(u)\mathrm du,\quad u=\varphi(x)
$$
...
$$
\begin{align*}
\int f(ax+b)\mathrm dx&=\frac1a\int f(u)\mathrm du\\
\int f(x^n)x^{n-1}\mathrm dx&=\frac1n\int f(u)\mathrm du\\
\int f(e^x)e^x\mathrm dx&=\int f(u)\mathrm du\\
\int f(\ln x)\frac1x\mathrm dx&=\int f(u)\mathrm du\\
\int f(\sin x)\cos x\mathrm dx&=\int f(u)\mathrm du\\
\int f(\tan x)\sec^2x\mathrm dx&=\int f(u)\mathrm du
\end{align*}
$$
...
**常用选择顺序（优先选 $u$）**
**反三角函数 > 对数函数 > 幂函数 > 指数/三角函数**

典型：
$$
\begin{align*}
\int x e^x\mathrm dx&=x e^x-e^x+C\\
\int x\sin x\mathrm dx&=-x\cos x+\sin x+C\\
\int \ln x\mathrm dx&=x\ln x-x+C\\
\int \arctan x\mathrm dx&=x\arctan x-\frac12\ln(1+x^2)+C
\end{align*}
$$
