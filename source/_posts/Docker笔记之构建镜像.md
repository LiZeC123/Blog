...
使用这种方式可以更好的共享和复用资源, 例如许多镜像可能都依赖` ubuntu:22.04`镜像, 那么本地只需要存储一份该镜像, 即可在所有的镜像文件中共享. 
...
判断是否缓存的逻辑是计算上一层的哈希值+本层指令的哈希值. 如果两者均为发生变化, 则本层可以直接复用. 当然, 对于`ADD`和`COPY`等涉及从文件内容的指令, Docker也会计算文件的哈希值, 在确保文件也没有发生变化时才会复用对应的层.
