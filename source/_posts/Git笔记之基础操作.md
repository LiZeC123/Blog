...
第三条指令将git默认的编辑器设置为vim. 第四条指令配置当Git创建一个新分支后第一次推送远程时, 自动在远程创建同名分支并关联.
...
```sh
git blame -L 42,42 src/main.js
```

使用以上指令可以查看`src/main.js`第`42`行上一次的修改人. 用于此功能过于常用, 因此通常可以安装插件实现, 例如`Git Blame`.

### 查看修改历史

```sh
# 只查看提交记录, 不查看diff
git log -L 42,42:src/main.js --oneline

# 追踪重命名 / 移动过的行
git log -L 42,42:src/main.js -M -C
```

这一功能在IDEA等IDE中均以封装为右键菜单的指令, 但在Vscode中并没有某个插件可以简单的支持. 如果偶尔需要回溯一下某几行代码的变更历史, 那么直接用命令行是更简单的选择.
