...
Go语言在标准库`database/sql包中提供了数据库相关的操作. 在`database/sql包中并未实现具体的数据库操作, 而是给出了一组关于`driver`的接口, 所有关于数据库的操作最终均通过操作`driver`的接口实现. 通过在代码中注册不同的数据库driver, 从而实现连接到不同的数据库.
