...
任何实现了 `__iter__()` 和 `__next__()` 方法的对象都是迭代器（Iterator）。迭代器的核心特点是 一次性消费，遍历完后无法重置。生成器（Generator）是迭代器的子类型，由含有 yield 关键字的函数或生成器表达式创建。除了 `__next__()`，它还额外拥有 `send()`、`throw()`、`close()` 方法。


```py
def gen():
    received = yield "准备接收"   # 先产出字符串，然后等待 send 进来的值
    print(f"收到了: {received}")
    yield received * 2

g = gen()
value1 = next(g)          # 或者 g.send(None)，启动到第一个 yield
print(value1)             # 输出 "准备接收"

value2 = g.send(42)       # 发送 42 给生成器，赋值给 received
print(value2)             # 输出 84
```

> 加上这些方法以后, Generator就从一个只能输出的东西变成了一个完整的pipe, 从而可以实现流计算.
...
```text
Iterable (定义 __iter__)
    ↑
Iterator (定义 __next__, 并且 __iter__ 返回自身)
    ↑
Generator (额外定义 send, throw, close)
```

`Iterable`：仅承诺可被 `for` 循环迭代，不保证一次/多次，也不保证有 `__next__` 或长度。`Iterator`：承诺可调用 `next()` 逐步取值，通常只能迭代一次。


| 场景                                    | 推荐注解             | 理由                               |
| --------------------------------------- | -------------------- | ---------------------------------- |
| 只需要 `for` 循环遍历，不关心是否一次性 | `Iterable[T]`        | 最灵活，可接受列表、元组、生成器等 |
| 需要手动 `next()` 或强调“一次性消费”    | `Iterator[T]`        | 明确禁止重复迭代，适合流式数据     |
| 需要 `send`/`throw` 功能（如协程）      | `Generator[Y, S, R]` | 暴露完整生成器接口                 |

---

`Sequence` 是 `Iterable` 的子类，额外要求支持 `__len__` 和 `__getitem__`（整数索引）。`list`、`tuple`、`str` 都是 `Sequence`。

```python
from collections.abc import Sequence

def second_element(seq: Sequence[int]) -> int:
    return seq[1]          # 索引合法，且 len(seq) 保证存在

def print_all(it: Iterable[int]) -> None:
    for x in it:
        print(x)
    # 但不能做 len(it) 或 it[0]
```

选择 `Sequence` 当且仅当你的函数需要随机访问或获取长度。否则优先用 `Iterable`，保持灵活。


> Python 3.5 引入 `typing` 模块支持类型提示，其中包含了 `Iterable`、`Sequence` 等泛型版本。但 Python 3.9（PEP 585）后，**原生容器和 `collections.abc` 中的抽象基类都支持泛型**，因此官方决定逐步弃用 `typing` 中的同名类型。
