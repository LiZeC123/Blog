...
执行`go get`指令实际上就是先从指定的URL下载对应的代码, 将依赖的源代码复制到`$GOPATH/pkg`的指定位置. Go执行编译操作时会在`$GOPATH/pkg`中查找依赖的源码并进行编译和链接. Go的编译过程会启用缓存, 对于一个给定的依赖, 再次编译时不会从源码编译, 而是直接查询缓存, 如果命中缓存则可以直接链接对应的二进制库文件.

> 正是因为这种从源码编译并启用缓存的机制, Go才能既保证较快的编译速度, 又保证编译时可以进行依赖替换.

`go get`指令也可以用于安装第三方工具, 但现在建议使用`go install`指令来实现此功能.
...
> 注意: 建议包名和路径名完全保持一致, 否则在单个Git仓库中存储多个模块的场景, 可能出现无法根据URL加载依赖的情况
...
`go install`指令类似于`make install`, 主要目的是将可执行文件复制到`$GOPATH/bin`位置之中, 从而使对应的可执行文件能够被命令行直接访问. 对于第三方工具, 也可以使用`go install`指令直接从URL获取并完成编译和安装.
...
通常情况下, 一个Go模块对应一个Git仓库, 此时模块名就是Git仓库的URL, 版本号就是Git的Tag. 但也存在一个Git仓库中存储多个模块的场景, 此时新增Tag时需要准时路径规则, 即

例如模块`golang.org/x/tools/gopls`存储在该仓库的`gopls`子目录下, 此时的根路径是`golang.org/x/tools`. 那么在发布`v0.4.0`时, tag应该命名为`gopls/v0.4.0`, 其中的前缀是该依赖相对根目录的路径.

> [Mapping versions to commits - The Go Programming Language](https://go.dev/ref/mod#vcs-version)
...
> 使用返回值变量命名以后, 需要注意是否**重复定义**了名称类似的变量, 以免最后返回的值不符合预期
...
否则会将切片作为`一个`参数(例如一个interface的数组)传入函数之中.
...
被defer修饰的语句会在当前模块执行return语句之前执行. defer语句通常用来实现关闭资源等操作, 在开启资源后紧跟一个defer引导的关闭语句, 从而避免忘记关闭资源.

但需要注意, defer语句本身需要先能够被执行, 才能保证在函数的最后执行指定的函数.
...
如果defer的内容在任意情况下均需要执行, 则建议紧跟执行函数. 如果defer的内容是关闭资源等操作, 则可以在异常分支的逻辑后执行(例如开启文件等). 因为资源开启失败的情况下无需关闭, 所以异常分支中直接返回即可.
...
上述代码中虽然在return之前a已经修改为2, 但是defer的时候已经拷贝了a的值, 因此输出的还是1. 对于map类型, 也是相同的道理, defer时直接引用值会导致后续对map的修改不可见.

> 当然, 如果变量是引用类型, 那么拷贝以后引用相同的对象, 因此可以见到修改后的结果. 
...
由于Go中实际上并不存在类, 因此如果一个结构体需要初始化函数, 通常创建一个New函数, 例如
```go
type File struct {
    fd      int     // 文件描述符
    name    string  // 文件名
...
// 由于包名的存在, 在不冲突的情况下, 简单的使用New命名即可
func New(fd int, name string) *File {
    if fd < 0 {
        return nil
    }
...
由于Go不像Java一样提供构造函数, 因此无法强制要求调用构造函数, 而直接声明的变量仅具有默认的零值. 对于引用类型, 零值相当于空指针. 为了避免产生空指针的问题, Go哲学强调零值可用, 例如
...
对于没有任何成员变量的结构体, Go进行了特殊优化, 该结构体不占据任何空间. 将空结构体作为其他结构体的成员时, 也不会实际占据空间.
...
对于不需要值的map(即需要一个set)或者使用Channel传递的消息体不需要实际内容时, 均可以使用空结构体来优化空间占用.
...
绑定在值类型上的方法和绑定在指针类型上的方法属于不同的集合. 但用方法时Go会自动转换, 因此无论调用方是值类型还是指针类型都可以直接调用. 不建议同时绑定值类型和指针类型的方法, 应该始终只选择其中的一种进行绑定. 处于性能考虑, 通常仅在指针类型上绑定方法来模拟类的效果.
...
Go语言无需明确声明实现了哪一个接口, 只要某个结构体实现了一个接口声明的所有方法, 则可以直接视为实现了该接口. 相比于Java这种需要明确声明实现接口的语言, 虽然按照Go的写法, 语法层面上想要确定一个类实现了什么接口有点困难, 但反正写代码都是用IDE, 所以有了IDE的辅助以后, 也没有太大的问题.
...
> 从GO1.18开始, 内置了`any`关键字, 其等价于上面的`interface{}`. 不过在用上泛型之前, 这也不过是一种勉强可用的东西.
> 而Go的泛型做的复杂度太高了, 目前看并不值得为了用上泛型引入这一套机制. 具体可参考 [Go 1.18 泛型全面讲解：一篇讲清泛型的全部](https://segmentfault.com/a/1190000041634906)
...
使用`go get`指令安装依赖时可以指定版本号. 根据版本号的情况, Go会自动调整相关依赖的版本, 实现依赖的升级或者降级. 以降级为例, 假设主模块依赖`A 1.2`, 模块`A 1.2`依赖`B 1.2`, 可通过`go get`指令对`B`进行降级. 降级后, `A`由于依赖`B`的高版本, 也会一起降级到低版本.

> 对于某些必须使用低版本的场景, 使用以上方式可以更好的保证依赖项目的一致性.

使用`go get xxx@none`可以移除某个依赖. 使用这种方式会重新计算依赖关系, 移除其他间接的依赖.
...
GO语言作为一个比较晚出现的语言, 天生具备跨平台和交叉编译的能力. 在不启用CGO特性时, 可以轻松的完成交叉编译, 只需要指定

```sh
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build 
```

即可轻松的在其他平台编译出可以在linux平台运行的二进制文件. 直接将文件复制到机器上即可执行.