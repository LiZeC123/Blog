...
与Python和C语言类似，Go中每一个文件都是一个代码组织的基本单元。对于每个文件，都可以定义一个init函数。此函数将会在此文件所在的模块第一次导入时自动调用，从而对模块执行一些初始化操作。

不同模块之间存在依赖关系，Go语言保证最底层的依赖中的init函数最先被调用，且无论被依赖多少次，init函数仅调用一次。

由于一个模块可以有多个文件，每个文件都可以有init函数，因此同一个模块内的多个init函数之间的调用顺序并不确定。
