...
MySQL采取默认提交(AUTOCOMMIT)模式, 即除非显式的开始一个事务, 则每个语句都视为一个事务, 在执行后立即提交. 可以使用`BEGIN`或者`START TRANSACTION`语句开启一个事务。如果使用`START TRANSACTION`开启事务，还可以跟随几个关键词来指定具体的事务类型
