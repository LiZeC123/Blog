...
**持久性**表明一个事务的操作一旦提交到数据库, 则即使数据库崩溃, 数据也不会丢失. 但实际上并不存在可以保证绝对不丢失数据的数据库, 因而持久性也存在不同的等级.

SQL标准规定了四种隔离级别, 每种级别都规定了一个事务的修改, 在其他事务内的可见性. 低级别的隔离通常具有更高的并发性并且系统开销也更低. 
...
可以使用`BEGIN`或者`START TRANSACTION`语句开启一个事务。如果使用`START TRANSACTION`开启事务，还可以跟随几个关键词来指定具体的事务类型
