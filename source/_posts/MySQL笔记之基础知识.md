...
InnoDB是MySQL默认的事务存储引擎, 它被设计用来处理大量短期事务, 此类事务的特点是大部分时候事务都正常提交, 很少被回滚. 

InnoDB较小的性能开销和自动崩溃恢复使功能得非事务任务中也可以考虑使用InnoDB来提高数据的安全性. 
...
NULL值列表: 记录每一行记录中值为NULL的字段, 由于SQL语法可以规定某个列不可为NULL, 因此这一列表可以不存在. 如果这一列表存在, 则每个可以为NULL的字段对应一个二进制位.
...
每个数据页的大小都是固定的16KB，当存储的记录不足16KB时，也会通过空白的方式占满16KB的空间。
...
用户的数据首先分割为不同的槽, 每个槽中可以包含4~8条数据, 页目录中存储了每个槽中最大的记录在页面中的偏移地址. 
