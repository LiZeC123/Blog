...
snap是Ubuntu系统自带的一个包管理器, 可以代替apt安装更多应用. 由于snap的包由开发者直接控制, 因此通常版本更新. 使用`snap find`可以查找需要的包. snap 有三种级别:

| confinement          | 沙箱隔离 | 系统权限                     | 安装要求                    | 典型用途                        |
| -------------------- | -------- | ---------------------------- | --------------------------- | ------------------------------- |
| **`strict`（默认）** | ✅ 强隔离 | 最小权限，仅通过接口显式授权 | 无需额外参数                | 普通应用（浏览器、办公、工具）  |
| **`classic`**        | ❌ 无沙箱 | 全系统访问（同传统 deb）     | 必须加 `--classic` 才能安装 | IDE、编译器、系统工具、开发环境 |
| `devmode`            | 半隔离   | 宽松权限 + 日志审计          | 开发调试用                  | 开发者打包测试                  |


对于classic类型的应用, 必须使用`--classic`参数安装.
...
```bash
ffmpeg -i input.mp4  -c:v libx265 -preset medium -crf 26 -c:a copy output_1080p.mp4
```


`-i`参数指定输入文件. 如果仅指定`-i`则ffmpeg仅输出文件的元信息(编码格式, 分辨率, 码率等). `-c:v`指定视频编码格式, 常见选项包含

| 编码格式 | CPU编码    | NVIDIA显卡编码 | Mac硬件编码       |
| -------- | ---------- | -------------- | ----------------- |
| H.264    | libx264    | h264_nvenc     | h264_videotoolbox |
| H.265    | libx265    | hevc_nvenc     | hevc_videotoolbox |
| AV1      | libaom-av1 | av1_nvenc      | M1~M3不支持       |


> 1. 使用纯CPU编码时, H.265在双核心服务器上只有x0.4左右的编码速度. 而AV1比H265格式还要再慢3~5倍, 只有不到x0.1的速度. 
> 2. 使用NVIDIA显卡时, 对所有类型的编码均可以提供x17倍的编码速度
> 3. 使用Mac平台硬件可以对H.265提供x6倍的编码速度. 虽然Mac的转码速度比显卡慢, 不过确实能耗比较低, 不会导致发烫, 也不影响同时做其他事情
...
> `-crf`控制了画面的质量, 决定了最后视频文件的体积大小, 而`-preset`控制了编码的速度, 牺牲速度可以在同样的画面质量下获得更小的体积. `-preset`对于显卡编码意义不大, 保持默认即可.
...
1. 转码操作前需要先确认原视频的码率, 如果原视频码率已经非常低了, 那么即使转换为其他格式, 收益也不大(甚至可能视频文件会变大)
2. 对于核心数量很少的场景, 串行转码多个文件的速度要大于并行转码. 因为同时操作多个不同的文件加剧了缓存不命中的情况
3. Mac平台硬件编码不支持恒定质量因子, 必须手动指定码率
...
```bash
ffmpeg -i input.mp4 -c:v libx265 -b:v 2500k -c:a copy output.mp4
```
