...
不同类型的选择器之间, 权重越大的选择器, 其有限期越高. 权值相等时, 后出现的样式表设置要优于先出现的样式表设置. 高权重的样式可以覆盖低权重的样式.
...
absolute按照父元素进行移动, 如果没有父元素, 或祖先元素没有设置定位, 则相对于浏览器文档进行移动. 如果祖先元素有定位, 则按照最近的有定位的祖先元素作为参考点.

相对定位和绝对定位的口诀是:**子绝父相**. 即子元素使用绝对定位时, 父元素应该使用相对定位. 
